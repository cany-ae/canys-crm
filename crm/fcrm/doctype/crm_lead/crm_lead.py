...
# ============================================================
# Permission: Vertriebler sieht nur eigene Leads (lead_owner)
# GF (Sales Manager) sieht alle
# ============================================================

def get_permission_query_conditions(user):
    """Filter CRM Lead list queries based on role.

    Sales Manager / System Manager / Administrator: see all leads.
    Everyone else (Sales User / Vertriebler): only leads where lead_owner = user.
    """
    if not user:
        user = frappe.session.user
    ...
    # Vertriebler: nur eigene Leads
    return "(`tabCRM Lead`.lead_owner = {user} OR `tabCRM Lead`.lead_owner = '' OR `tabCRM Lead`.lead_owner IS NULL)".format(
        user=frappe.db.escape(user)
    )


def has_lead_permission(doc, ptype, user):
    """Check single-document permission for CRM Lead.

    Sales Manager / System Manager / Administrator: full access.
    Everyone else: only if lead_owner matches or is unassigned.
    """
    if not user:
        user = frappe.session.user
    ...
    # Vertriebler: nur eigene Leads oder nicht zugewiesene
